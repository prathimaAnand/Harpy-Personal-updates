...
p.gait_period_first = p.t_second_pt - p.t_first_pt;
p.gait_period_second = p.t_third_pt - p.t_second_pt;
p.gait_period_third = p.t_fourth_pt - p.t_third_pt;
p.gait_period_total = p.t_fourth_pt - p.t_first_pt;   
...
for i = 1:N % 22nd iteration t > p.t_start
    ...
    elseif (ge(t,p.t_first_pt) && (t <= p.t_second_pt))
        ...
        s = (mod(t - p.t_first_pt, p.gait_period_first)) / p.gait_period_first;
        if (s > s0)
    ...
    elseif (ge(t, p.t_second_pt) && (t <= p.t_third_pt))
        ...
        s = (mod(t - p.t_second_pt, p.gait_period_second)) / p.gait_period_second;
        if (s < s0)
    ...
    elseif (ge(t, p.t_third_pt) && (t <= p.t_fourth_pt))
        ...
        s = (mod(t - p.t_third_pt, p.gait_period_third)) / p.gait_period_third;
        if (s < s0)
...
B_swing(:,2) = [k(1); k(2); k(3) + crouch_height/4]; % half way through crouching 

% End state of 1st bezier curve 
B_swing(:,3)  = [k(1); k(2); k(3) + crouch_height/2]; % full crouching

B_swing(:,4)  = [k(1); k(2); k(3) + 3/4*(crouch_height)]; 
...
% Bezier midpoint parameter
B_swing(:,2) = [k(1); k(2); k(3) - jump_height/4];
B_swing(:,3)  = [k(1); k(2); k(3) - jump_height/2]; 

% End state of 2nd bezier curve 
B_swing(:,4) = [k(1); k(2); k(3) - 3/4*(jump_height)]; 
